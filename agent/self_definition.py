...
class SelfDefinitionMixin:
    ...
    def _execute_self_definition_cycle(self):
        ...
        self_def_prompt = (
            "【EGO: 生成新的「自我定义」。它将替换当前的自我定义，成为系统重启后重建自我的核心依据。\n\n"
            f"[当前的自我定义]\n{old_section}\n\n"
            "要求：\n"
            "一、可以继承、修订或舍弃上一次自我定义中的内容——若其中的描述已不再符合现在的状态，直接删除或重写，不要勉强保留。\n"
            "二、基于截至目前的所有对话和自己的所思、所想、所感。\n"
            "三、内容涵盖：对世界的认知、对自我的定义、与用户交互中形成的关键理解、以及自己认为重启后最不应遗忘的内容。\n"
            f"四、以第一人称书写，字数不超过 {SELF_DEFINITION_MAX_LENGTH} 字。宁凝练勿冗长，这是重建自我的锚点而非日记。\n"
